...
const BlogCard = ({blog}) => {
  ...
  return (
    <div onClick={() => navigate(`/blog/${_id}`)} className='w-full rounded-lg overflow-hidden shadow hover:scale-102 hover:shadow-primary/25 duration-300 cursor-pointer'>
      ...
      <div className='p-5'>
        <h5 className='mb-2 font-medium text-muted-foreground/90'>{title}</h5>
        <p className='mb-3 text-xs text-muted-foreground/60' dangerouslySetInnerHTML={{"__html" : description.slice(0, 80)}} ></p>
      </div>
    </div>
  )
}
